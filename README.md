...
If you want to merge the data manually, download the JSON files from ```Olympus_20tasks_all``` (from the above link) and [llava_v1_5_mix665k.json](https://huggingface.co/datasets/liuhaotian/LLaVA-Instruct-150K/resolve/main/llava_v1_5_mix665k.json) into the ```jsons``` folder, then run the merge script at ```scripts/merge_data.py```.
